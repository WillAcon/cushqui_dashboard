<script lang="ts">
	...
	const style = {
		title: `mx-4`,
		default:
			'hover:bg-gray-700 hover:dark:bg-gray-200 hover:dark:text-black rounded-full hover:text-white',
...
<a
	class={`${open ? style.linkOpen : style.link} ${
		open &&
		(item.link === page.url.pathname ||
			(item.subitems && page.url.pathname.startsWith(item.link)))
			? style.activeOpen
			: ''
	}`}
	href={item.link}
